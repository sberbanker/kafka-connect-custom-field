...
import ru.rgs.kafka.connect.transforms.TimestampConverter;
...
public abstract class TimestampConverter<R extends ConnectRecord<R>> implements Transformation<R> {
    ...
    public static final ConfigDef CONFIG_DEF = new ConfigDef()
            .define(FIELDS_CONFIG, ConfigDef.Type.STRING, FIELD_DEFAULT, ConfigDef.Importance.HIGH,
            ...
            .define(TIMEZONE_CONFIG, ConfigDef.Type.STRING, FORMAT_DEFAULT, ConfigDef.Importance.LOW,
                    "A Timezone name in IANA-compatible format or `system` for server timezone. UTC is default.");
    ...
    public void configure(Map<String, ?> configs) {
        ...
        if (isBlank(timezone)) {
            ...
            try {
                zoneId = timezone.equals("system") ? ZoneId.systemDefault() : ZoneId.of(timezone);
            } catch (DateTimeException e) {
                throw new ConfigException("TimestampConverter requires a IANA-compatible pattern for timezone: " + formatPattern + " or `system` value.", e);
            }
        }
        ...
        if (!isBlank(formatPattern)) {
            try {
                ...
                throw new ConfigException("TimestampConverter requires a DateTimeFormatter-compatible pattern for string timestamps: " + formatPattern, e);
            }
        }
    ...
    private interface TimestampTranslator {
        /**
         * Convert from the type-specific format to the universal java.time.DateTime format
         */
    ...
    static {
        TRANSLATORS.put(TYPE_STRING, new TimestampTranslator() {
            ...
            public Date toRaw(Config config, Object orig) {
                ...
                try {
                    return Date.from(config.dateTimeFormatter.parse((String) orig, Instant::from));
                } catch (DateTimeParseException e) {
                    throw new DataException("Could not parse timestamp: value (" + orig + ") does not match pattern (" + config.pattern + ")", e);
                }
            }
            ...
            public String toType(Config config, Date orig) {
                synchronized (config.dateTimeFormatter) {
                    return config.dateTimeFormatter.format(orig.toInstant());
                }
            }
        });
